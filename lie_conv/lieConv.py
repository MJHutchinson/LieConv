...
class PointConv(nn.Module):
    def __init__(
        ...
        self.weightnet = WeightNet(
            xyz_dim, self.cmco_ci, act, bn
        )  # MLP - final layer to compute kernel vals (see A1)
        self.linear = nn.Linear(
            self.cmco_ci * chin, chout
        )  # final linear layer to compute kernel vals (see A1)
        self.mean = mean  # Whether or not to divide by the number of mc_samples
        assert (
            ds_frac == 1
        ), "test branch no ds, will need to carefully check that subsample respects padding"
        self.subsample = FarthestSubsample(
            ds_frac, knn_channels=knn_channels
        )  # Module for subsampling if ds_frac<1
    ...
    def extract_neighborhood(self, inp, query_xyz):
        ...
        neighbor_idx = knn_point(
            min(self.mc_samples, inp_xyz.shape[1]),
            inp_xyz[:, :, : self.knn_channels],
            query_xyz[:, :, : self.knn_channels],
            mask,
        )
    ...
    def point_convolve(self, embedded_group_elems, nbhd_vals, nbhd_mask):
        ...
        (
            ...
            nbhd,
            ci,
        ) = nbhd_vals.shape  # (bs,m,mc_samples,d) -> (bs,m,mc_samples,cm*co/ci)
        _, penult_kernel_weights, _ = self.weightnet(
            (None, embedded_group_elems, nbhd_mask)
        )
        penult_kernel_weights_m = torch.where(
            nbhd_mask.unsqueeze(-1),
            penult_kernel_weights,
            torch.zeros_like(penult_kernel_weights),
        )
        nbhd_vals_m = torch.where(
            nbhd_mask.unsqueeze(-1), nbhd_vals, torch.zeros_like(nbhd_vals)
        )
        #      (bs,m,mc_samples,ci) -> (bs,m,ci,mc_samples) @ (bs, m, mc_samples, cmco/ci) -> (bs,m,ci,cmco/ci) -> (bs,m, cmco)
        partial_convolved_vals = (
            nbhd_vals_m.transpose(-1, -2) @ penult_kernel_weights_m
        ).view(bs, m, -1)
        convolved_vals = self.linear(
            partial_convolved_vals
        )  #  (bs,m,cmco) -> (bs,m,co)
        if self.mean:
            convolved_vals /= nbhd_mask.sum(-1, keepdim=True).clamp(min=1)
    ...
    def forward(self, inp):
        ...
        convolved_wzeros = torch.where(
            sub_mask.unsqueeze(-1), convolved_vals, torch.zeros_like(convolved_vals)
        )
...
def FPSindices(dists, frac, mask):
    ...
    a = torch.randint(
        0, n, (bs,), dtype=torch.long, device=device
    )  # choose random start
    idx = a % mask.sum(-1) + torch.cat(
        [torch.zeros(1, device=device).long(), torch.cumsum(mask.sum(-1), dim=0)[:-1]],
        dim=0,
    )
    ...
    for i in range(m):
        ...
        dist = torch.where(
            mask, dists[B, farthest], -100 * torch.ones_like(distances)
        )  # (bs,n) compute distance from new point to all others
        closer = (
            dist < distances
        )  # if dist from new point is smaller than chosen points so far
        distances[closer] = dist[
            closer
        ]  # update the chosen set's distance to all other points
        farthest = torch.max(distances, -1)[
            1
        ]  # select the point that is farthest from the set
        ...
        if self.ds_frac != 1:
            if self.cache and self.cached_indices is None:
                query_idx = self.cached_indices = FPSindices(
                    dist(abq_pairs), self.ds_frac, mask
                ).detach()
            elif self.cache:
                query_idx = self.cached_indices
            else:
                query_idx = FPSindices(dist(abq_pairs), self.ds_frac, mask)
            B = torch.arange(query_idx.shape[0], device=query_idx.device).long()[
                :, None
            ]
            ...
            query_idx = None
        if withquery:
            return (subsampled_abq_pairs, subsampled_values, subsampled_mask, query_idx)
        return (subsampled_abq_pairs, subsampled_values, subsampled_mask)


class LieConv(PointConv):
    def __init__(
        self, *args, group=SE3, ds_frac=1, fill=1 / 3, cache=False, knn=False, **kwargs
    ):
        ...
        self.coeff = 0.5  # Internal coefficient used for updating r
        self.fill_frac_ema = (
            fill  # Keeps track of average fill frac, used for logging only
        )
    ...
    def extract_neighborhood(self, inp, query_indices):
        ...
        dists = torch.where(
            mask[:, None, :].expand(*dists.shape), dists, 1e8 * torch.ones_like(dists)
        )
        ...
        if self.knn:  # NBHD: KNN
            nbhd_idx = torch.topk(dists, k, dim=-1, largest=False, sorted=False)[
                1
            ]  # (bs,m,nbhd)
            valid_within_ball = (
                (nbhd_idx > -1) & mask[:, None, :] & mask_at_query[:, :, None]
            )
            ...
            bs, m, n = dists.shape
            within_ball = (
                (dists < self.r) & mask[:, None, :] & mask_at_query[:, :, None]
            )  # (bs,m,n)
            B = torch.arange(bs)[:, None, None]
            M = torch.arange(m)[None, :, None]
            noise = torch.zeros(bs, m, n, device=within_ball.device)
            noise.uniform_(0, 1)
            valid_within_ball, nbhd_idx = torch.topk(
                within_ball + noise, k, dim=-1, largest=True, sorted=False
            )
            valid_within_ball = valid_within_ball > 1

        # Retrieve abq_pairs, values, and mask at the nbhd locations
        B = (
            torch.arange(inp_vals.shape[0], device=inp_vals.device)
            .long()[:, None, None]
            .expand(*nbhd_idx.shape)
        )
        M = (
            torch.arange(abq_at_query.shape[1], device=inp_vals.device)
            .long()[None, :, None]
            .expand(*nbhd_idx.shape)
        )
        ...
        if (
            self.training and not self.knn
        ):  # update ball radius to match fraction fill_frac inside
            ...
            self.fill_frac_ema += 0.1 * (avg_fill - self.fill_frac_ema)
    ...
    def point_convolve(self, embedded_group_elems, nbhd_vals, nbhd_mask):
        ...
        if self.mean:
            convolved_vals /= nbhd_mask.sum(-1, keepdim=True).clamp(
                min=1
...
class BottleBlock(nn.Module):
    ...
    def __init__(self, chin, chout, conv, bn=False, act="swish", fill=None):
        ...
        assert (
            chin <= chout
        ), f"unsupported channels chin{chin}, chout{chout}. No upsampling atm."
        nonlinearity = Swish if act == "swish" else nn.ReLU
        self.conv = (
            conv(chin // 4, chout // 4, fill=fill)
            if fill is not None
...
class LieResNet(nn.Module, metaclass=Named):
    ...
    def forward(self, x):
        ...
        if self.lie_algebra_nonlinearity is not None:
            lifted_x = list(lifted_x)
            pairs_norm = lifted_x[0].norm(dim=-1) + 1e-6
            lifted_x[0] = lifted_x[0] * (
                self.lie_algebra_nonlinearity(pairs_norm / 7) / pairs_norm
            ).unsqueeze(-1)

        # print("max norm: ", lifted_x[0].norm(dim=-1).max())
...
class ImgLieResnet(LieResNet):
    ...
    def forward(self, x, coord_transform=None):
        ...
        coords = (
            coords[center_mask].view(-1, 2).unsqueeze(0).repeat(bs, 1, 1).to(x.device)
        )
